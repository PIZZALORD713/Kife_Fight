function getButtonStyle(isStunned, promptPhase, promptType) {
  ...
  if (promptPhase === 'active') {
    ...
    if (promptType === 'CHARGE') return 'bg-amber-600 hover:bg-amber-500 active:scale-95';
...
function getButtonLabel(isStunned, promptPhase, promptType, isHolding, isCharging, combo) {
  if (isStunned) return '💫 STUNNED';
  if (promptPhase === 'active') {
    if (promptType === 'HOLD')   return isHolding ? '✊ HOLDING...' : '✋ PRESS & HOLD';
    if (promptType === 'PAUSE')  return '🧘 STAY STILL';
    if (promptType === 'CHARGE') return isCharging ? '⚡ RELEASE IN GREEN!' : '⚡ PRESS & CHARGE';
...
export function AttackButton({ combo = 1, isStunned, promptPhase, promptType, isHolding, isCharging, onPointerDown, onPointerUp, onPointerLeave }) {
  ...
  return (
    <div className="shrink-0 flex flex-col gap-1.5">
      ...
      >
        {getButtonLabel(isStunned, promptPhase, promptType, isHolding, isCharging, combo)}
      </button>
